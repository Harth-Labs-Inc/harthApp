...
const saveBlockedId = (db, data, id) => {
  return new Promise((resolve) => {
    ...
    const blockedUser = { userId: id, blockedDate: currentDate };

    const updateOperation = {
      $addToSet: { BlockedList: { $each: [blockedUser] || [] } },
    };
...
export default async (req, res) => {
  ...
  const { ActiveUser, userIdToBlock } = obj;
  ...
  // Authentication end
  let newUser = await saveBlockedId(db, ActiveUser, userIdToBlock);
