...
import { getTurnServers } from "requests/rooms";
...
const Stream = ({
  ...
  useEffect(() => {
    if (badAuth) {
      window.location.replace("/auth/createAccount");
    }
  }, [badAuth]);
  ...
  if (!badAuth) {
    ...
    const [selectedHarth, setSelectedHarth] = useState(harthProp);
    ...
    const [userIcon, setUserIcon] = useState(userIconProp);
    const [roomId, setRoomId] = useState(roomIdProp);
    const [harthId, setHarthId] = useState(harthIdProp);
    const [isActiveScreenShare, setIsActiveScreenShare] = useState(false);
    const [TurnServers, setTurnServers] = useState(turnServersProp);
...
export async function getServerSideProps(context) {
  ...
  if ("harth_id" in queryObject) {
    const [harthResult, turnServerResult] = await Promise.all([
      getHarthByID(queryObject["harth_id"], authToken),
      getTurnServers(authToken),
    ]);
    const { data } = harthResult;
    let creator = data?.users.find((usr) => usr.userId === user._id);
    const harthProp = data;
    const roomIdProp = queryObject["room_id"];
    const harthIdProp = queryObject["harth_id"];
    const userIconProp = creator?.iconKey;
    const userNameProp = creator?.name;
    const userIDProp = user._id;
    const turnServersProp = turnServerResult.servers.token.iceServers;
