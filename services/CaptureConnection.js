...
class SocketCaptureConnection {
  ...
  initializeSocketEvents = () => {
    ...
    this.socket.on('display-media', (data) => {
      if (data.value)
        this.checkAndAddClass(this.getMyVideo(data.userID), 'displayMedia')
      else this.checkAndAddClass(this.getMyVideo(data.userID), 'userMedia')
    })
  ...
  initializePeersEvents = () => {
    this.myPeer.on('open', async (id) => {
      ...
      this.socket.emit('join-capture-room', userData, this.setInitialMessages)
